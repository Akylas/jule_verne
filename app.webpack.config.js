...
NsVueTemplateCompiler.registerElement(
    "MDTextField",
    () => require("@nativescript-community/ui-material-textfield").TextField,
    {
        model: {
            prop: "text",
            event: "textChange"
...
NsVueTemplateCompiler.registerElement(
    "MDSlider",
    () => require("@nativescript-community/ui-material-slider").Slider,
    {
        model: {
            prop: "value",
            event: "valueChange"
...
module.exports = (env, params = {}) => {
    ...
    if (platform === "android") {
        env.appComponents = [
            resolve(projectRoot, "app/services/android/BgService.ts"),
            resolve(projectRoot, "app/services/android/BgServiceBinder.ts")
        ];
    }
    ...
    const nsConfig = nativescriptLib.projectDataService.getProjectData()
        .nsConfig;
    ...
    const mdiSymbols = symbolsParser.parseSymbols(
        readFileSync(
            resolve(projectRoot, "node_modules/@mdi/font/scss/_variables.scss")
        ).toString()
    );
    const mdiIcons = JSON.parse(
        `{${mdiSymbols.variables[mdiSymbols.variables.length - 1].value.replace(
            /" (F|0)(.*?)([,\n]|$)/g,
            '": "$1$2"$3'
        )}}`
    );
    const scssPrepend = `
        $mdi-fontFamily: ${platform === "android"
    ...
    const indexOfTsLoaderRule = config.module.rules.findIndex(
    ...
    config.plugins = config.plugins.filter(
        p =>
            ["DefinePlugin", "CleanWebpackPlugin", "CopyPlugin"].indexOf(
                p.constructor.name
            ) === -1
    );
    // we add our rules
        ...
        if (!!sentry && !!uploadSentry) {
            ...
            if (platform === "android") {
                appVersion = readFileSync(
                    join(appResourcesPath, "Android/app.gradle"),
                    "utf8"
                ).match(/versionName "((?:[0-9]+\.?)+)"/)[1];
                buildNumber = readFileSync(
                    join(appResourcesPath, "Android/app.gradle"),
                    "utf8"
                ).match(/versionCode ([0-9]+)/)[1];
            } else if (platform === "ios") {
                appVersion = readFileSync(
                    join(appResourcesPath, "iOS/Info.plist"),
                    "utf8"
                ).match(
    ...
    const isAnySourceMapEnabled =
        !!sourceMap || !!hiddenSourceMap || !!inlineSourceMap;
